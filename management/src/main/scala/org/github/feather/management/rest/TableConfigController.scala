...
import org.github.feather.management.service.TableConfigService
 ...
 */
@Singleton
class TableConfigController @Inject()(loggingAction: LoggingAction,
                                    service: TableConfigService,
                                    cc: ControllerComponents,
                                    implicit val ec: ExecutionContext) extends AbstractController(cc) {

  implicit val format: Format[TableConfigRow] = Json.format[TableConfigRow]
  implicit val dtoFormat: Format[TableConfigDTO] = Json.format[TableConfigDTO]

  def save: Action[JsValue] = loggingAction { implicit request: Request[JsValue] =>
    val value: JsResult[TableConfigRow] = request.body.validate[TableConfigRow]
    value.fold(
  ...
  def del: Action[JsValue] = loggingAction { implicit request: Request[JsValue] =>
    val value: JsResult[TableConfigDTO] = request.body.validate[TableConfigDTO]
    value.fold(
      err => BadRequest("JsonError"),
